...
; =============SwitchAcc Mod - Demen
	...
	If Number($iGoldLast) > Number($topgoldloot) Then
		If $topgoldloot > 0  And $ichkAlertTopGain  = 1 Then
    ...
    If Number($iElixirLast) > Number($topelixirloot) Then
		If $topelixirloot > 0 And $ichkAlertTopGain  = 1 Then
    ...
    If Number($iDarkLast) > Number($topdarkloot) Then
        If $topdarkloot > 0  And $ichkAlertTopGain  = 1 Then
	...
	If Number($iTrophyLast) > Number($topTrophyloot) Then
		If $topTrophyloot > 0  And $ichkAlertTopGain  = 1 Then
