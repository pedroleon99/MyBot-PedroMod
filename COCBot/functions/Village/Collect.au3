...
If ((Number($tempGold) <= Number($itxtTreasuryGold)) And $ichkCollectTresoryGold = 1) Or ((Number($tempElixir) <= Number($itxtTreasuryElixir)) And $ichkCollectTresoryElixir = 1) Or ((Number($tempDElixir) <= Number($itxtTreasuryDark)) And $ichkCollectTresoryDark = 1) Then
If (Number($tempGold) <= Number($itxtTreasuryGold)) And $ichkCollectTresoryGold = 1 Then
	SetLog ("Gold reached minimum" , $COLOR_PURPLE)
EndIf
If (Number($tempElixir) <= Number($itxtTreasuryElixir)) And $ichkCollectTresoryElixir = 1 Then
	SetLog ("Elixir reached minimum" , $COLOR_PURPLE)
EndIf
If (Number($tempDElixir) <= Number($itxtTreasuryDark)) And $ichkCollectTresoryDark = 1 Then
